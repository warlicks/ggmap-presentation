...
###############################################################################
## Name a Landmark
geocode(location = "White House", source = "google")

## Set an Address
address <- geocode(location = "2201 G St. NW, Washington, DC", source = "google")
address

## Create an Intersection
geocode(location = "G St NW & 22nd St NW, Washington, DC", source = "google")
map2 <- ggmap(get_map(location = c(lon = -98.01001, lat = 41.11103)))
map2  #Intersections Don't Seem to Work.

# Geocode a State
geocode(location = 'Iowa', source = "google")
map3 <- ggmap(get_map(location = c(lon = -93.0977, lat = 41.874), source = 'google', zoom = 7))
map3

## Geocode Several Locations At Once
cordinates <- geocode(location = c("Hartsfield - Jackson Atlanta International", "LAX", 'ORD', 'DFW', 'Denver International', 'JFK', 'SFO', 'CLT', 'LAS', 'PHX', 'Miami International', 'IAH', 'EWR', 'MCO', 'SEA', 'MSP', 'DTW', 'BOS', 'PHL', 'LGA'), source = "google")
